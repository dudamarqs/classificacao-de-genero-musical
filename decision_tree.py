...
NUMERIC_FEATURES = [
    ...
    "duration_minutes",
    "release_year",
    "track_popularity",  # Popularidade agora é feature, não alvo
]
...
# Parametros da Arvore de Decisao
#
# Os parametros abaixo foram mantidos identicos ao Random Forest sempre que
# o conceito existe nos dois algoritmos:
#
# max_depth=12
#   Limita a profundidade maxima da arvore. Controla diretamente o overfitting:
#   arvores muito profundas memorizam o treino. No RF esse papel e cumprido
#   por cada arvore individual do ensemble; aqui, regula a unica arvore.
#
# min_samples_leaf=2
#   Exige ao menos 2 amostras em cada folha. Evita folhas com um unico
#   exemplo, o que reduz ruido e melhora a generalizacao — mesmo papel
#   que no RF.
#
# max_features="sqrt"
#   Em cada no, considera apenas a raiz quadrada do total de features ao
#   buscar o melhor split. Introduz diversidade e evita que features muito
#   correlacionadas dominem todas as divisoes — identico ao RF.
#
# class_weight="balanced"
#   Ajusta os pesos de cada classe inversamente proporcional a sua frequencia.
#   Como "baixa" e mais frequente que "alta" neste dataset, o modelo recebe
#   penalizacao maior por errar na classe minoritaria — identico ao RF.
#
# random_state=42
#   Garante reproducibilidade nas escolhas aleatorias de max_features e nos
#   desempates entre splits de igual qualidade — identico ao RF.
#
# n_estimators nao se aplica: a Arvore de Decisao e um unico estimador.
# n_jobs     nao se aplica: sem paralelismo em um unico estimador.
...
# (identico ao main.py para garantir reproducibilidade)
...
# Pipeline da Arvore de Decisao

def build_feature_preprocessor() -> ColumnTransformer:
    """
    Cria o passo de pre-processamento usado antes do treino.

    Arvores de decisao sao invariantes a escala (apenas comparam thresholds),
    portanto StandardScaler nao e necessario — pipeline identica ao RF.

    - Colunas numericas: preenche faltantes com a mediana.
    - Colunas categoricas: preenche faltantes com o valor mais comum e
      depois transforma texto em colunas binarias via OneHotEncoder.
    """
...
def build_decision_tree_pipeline() -> Pipeline:
    """Cria a pipeline completa: pre-processamento + DecisionTreeClassifier."""
    return Pipeline(
        steps=[
...
def save_model_artifacts(
    ...
    confusion_matrix_path = METRICS_DIR / "decision_tree_confusion_matrix.csv"
    feature_importance_path = METRICS_DIR / "decision_tree_feature_importance.csv"
    predictions_path = METRICS_DIR / "decision_tree_test_predictions.csv"
    metrics_path = METRICS_DIR / "decision_tree_metrics.json"
    model_path = MODELS_DIR / "decision_tree_model.joblib"

    # Matriz de confusao dinamica para todos os generos presentes no teste
    ...
    # DecisionTreeClassifier possui feature_importances_, assim como o RF,
    # calculada pela reducao media de impureza (Gini) em cada feature.
...
def train_and_evaluate_model(
    ...
    """Treina a arvore de decisao, avalia no conjunto de teste e salva os artefatos."""
    ...
    metrics_summary["model_params"] = DECISION_TREE_PARAMS
